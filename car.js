...
const displayData=(datas)=>{
    console.log(datas)
    const carContainer=document.getElementById('car-conatiner')
    for(const data of datas)  {
        console.log(data)
const creatDiv=document.createElement('div')
creatDiv.innerHTML=`
<div class="card w-full glass">
  <figure><img src="${data.imageURL}" alt="car!"/></figure>
  <div class="card-body">
    <h2 class="card-title">Car NAme:${data.name}</h2>
    <p>Car Detsils: ${data.description}</p>
    <p>Tramission: </p>
    <div class="card-actions justify-end">
      <button class="btn btn-danger">Car Price: ${data.price}</button>
    </div>
  </div>
</div>
`
console.log(creatDiv)
carContainer.appendChild(creatDiv);
